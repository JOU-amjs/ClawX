    ...
    <div className={cn("relative flex min-h-0 flex-col -m-6 transition-colors duration-500 dark:bg-background")} style={{ height: 'calc(100vh - 2.5rem)' }}>
      ...
      {minLoading && !sending && (
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-background/20 backdrop-blur-[1px] rounded-lg pointer-events-auto">
          <div className="bg-background shadow-lg rounded-md p-2.5 border border-border">
...
function WelcomeScreen() {
  ...
  return (
    <div className="flex flex-col items-center justify-center text-center h-[60vh]">
      <h1 className="text-4xl md:text-5xl font-semibold text-foreground/80 mb-8 tracking-tight">
        {t('welcome.subtitle')}
      </h1>
      ...
      <div className="flex flex-wrap items-center justify-center gap-2.5 max-w-lg w-full">
        {quickActions.map(({ key, label }) => (
          <button 
            key={key}
            className="px-4 py-1.5 rounded-md border border-border text-[13px] font-medium text-foreground/70 hover:bg-muted/30 transition-colors bg-muted/30"
          >
            {label}
          </button>
        ))}
    ...
    <div className="flex gap-3">
      ...
      <div className="bg-muted/30 text-foreground rounded-lg px-4 py-3">
        <div className="flex gap-1">
          <span className="w-2 h-2 bg-muted-foreground/50 rounded-sm animate-bounce" style={{ animationDelay: '0ms' }} />
          <span className="w-2 h-2 bg-muted-foreground/50 rounded-sm animate-bounce" style={{ animationDelay: '150ms' }} />
          <span className="w-2 h-2 bg-muted-foreground/50 rounded-sm animate-bounce" style={{ animationDelay: '300ms' }} />
        </div>
      </div>
    </div>
  );
