...
function wechatStateDir(): string {
  return join(openclawDir(), 'openclaw-weixin');
}
...
function resolveConfigPath(): string {
  const envPath = process.env.OPENCLAW_STATE_DIR?.trim() || process.env.OPENCLAW_CONFIG?.trim();
  if (envPath) return join(envPath, 'openclaw.json');
...
export async function saveWeChatAccountState(rawAccountId: string, payload: {
  ...
  await mkdir(wechatAccountsDir(), { recursive: true });

  const filePath = join(wechatAccountsDir(), `${accountId}.json`);
  const data = {
