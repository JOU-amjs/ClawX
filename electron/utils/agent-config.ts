...
function getManagedWorkspaceDirectory(agent: AgentListEntry): string | null {
  ...
  const configuredWorkspace = expandPath(agent.workspace || join(getOpenClawConfigDir(), `workspace-${agent.id}`));
...
async function provisionAgentFilesystem(
  ...
  const targetWorkspace = expandPath(agent.workspace || join(getOpenClawConfigDir(), `workspace-${agent.id}`));
...
async function buildSnapshotFromConfig(config: AgentConfigDocument): Promise<AgentsSnapshot> {
  ...
  const agents: AgentSummary[] = entries.map((entry) => {
    ...
    return {
      ...
      inheritedModel,
      workspace: entry.workspace || (entry.id === MAIN_AGENT_ID ? getDefaultWorkspacePath(config) : join(getOpenClawConfigDir(), `workspace-${entry.id}`)),
...
export async function createAgent(
  ...
  return withConfigLock(async () => {
    ...
    const newAgent: AgentListEntry = {
      id: nextId,
      name: normalizedName,
      workspace: join(getOpenClawConfigDir(), `workspace-${nextId}`),
